...
def register_reporting_tools(mcp):
    ...
    async def create_report(
        ...
        tester: str = "Cybertool MCP Automated Assessment",
        scope: str = "",
        format: str = "html",
        filename: str = "",
    ) -> dict:
...
def _generate_markdown_report(title, target, findings, exec_summary,
    ...
    lines.extend([
        "",
        f"*Generated by Cybertool MCP Security Server | {date}*",
    ])
...
<p>{f.get('description','')}</p>"""
        ...
        html += "</div>"
    html += f"<footer style='text-align:center;color:#94a3b8;margin-top:2rem'>Generated by Cybertool MCP | {date}</footer></body></html>"
    return html
